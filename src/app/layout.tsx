...
interface RootLayoutProps {
  children: ReactNode; // children should be a ReactNode and is required
}
...
export default function RootLayout({ children }: RootLayoutProps) {
  ...
  // Safe access to localStorage with fallbacks and type safety
  useEffect(() => {
    const handleLocalStorage = () => {
      try {
        const authedUser = localStorage.getItem("authUser");
        ...
        if (authedUser) {
          const parsedUser: User | null = JSON.parse(authedUser);
        ...
        const usersList = localStorage.getItem("users") ?? "[]";
        if (!JSON.parse(usersList).length) {
